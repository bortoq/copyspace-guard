...
def metric_table(current: Report, candidate: Report, comparison: Dict[str, Any]) -> str:
    rows = [
    ...
    out = "| Metric | Current/baseline | Candidate/optimized |\n|---|---:|---:|\n"
...
def render_markdown(summary: Dict[str, Any]) -> str:
    ...
    cur = Report(**summary["reports"]["baseline"])
    cand = Report(**summary["reports"]["greedy"])
...
The baseline schedule is treated as the current or simple strategy. The candidate schedule is a deterministic greedy strategy.

{metric_table(cur, cand, comp)}
...
- `schedule_baseline.json` — baseline schedule.
- `schedule_greedy.json` — candidate schedule.
...
def render_html(summary: Dict[str, Any]) -> str:
    ...
    # Minimal markdown-to-HTML suitable for this controlled report.
    lines = md.splitlines()
    body = []
    in_ul = False
    in_table = False
...
<style>
:root {{ --bg:#0b1020; --card:#121a33; --text:#ecf2ff; --muted:#a8b3cf; --accent:#67e8f9; --good:#86efac; --line:#263154; }}
...
h1 {{ font-size:42px; line-height:1.05; margin:0 0 20px; }}
h2 {{ margin-top:34px; color:var(--accent); }}
p, li {{ color:var(--muted); font-size:16px; line-height:1.55; }}
.card {{ background:rgba(18,26,51,.86); border:1px solid var(--line); border-radius:18px; padding:26px; box-shadow:0 20px 60px rgba(0,0,0,.35); }}
...
<body><main><div class="badge">Metadata-only deterministic audit</div><div class="card">
{''.join(body)}
</div></main></body></html>
"""
